	...
	const overlay = mounted
		? createPortal(
				<div
					aria-hidden={!open}
					inert={!open}
					className="pointer-events-none fixed start-0 top-0 z-50 size-0"
				>
					<AnimatePresence
						initial={false}
						mode="popLayout"
					...
					>
						{open ? (
							<motion.div
								...
								className="fixed start-0 top-0 size-0"
							>
								<button
									type="button"
											...
											filteredItems.map((item, index) => {
												...
												return (
													<button
														...
														className="relative flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-start outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset"
													>
														{active ? (
															<motion.span
			...
			>
				{!open ? (
					<motion.button
						...
						className={cn(
							"flex size-full items-center rounded-xl bg-background/60 text-start backdrop-blur-md outline-none [--search-trigger-stroke:var(--color-border)] hover:[--search-trigger-stroke:var(--color-border-strong)] focus-visible:ring-2 focus-visible:ring-ring",
